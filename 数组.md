...
###swift语言可以通过Map、filter、Reduce对Array、Dictionary（等集合类型）进行操作
    map：转换。对数组中的元素进行转换。Map函数会遍历集合类型并对其中的每一个元素进行统一种操作，Map的返回值是一个所得结果的数组。


    filter：过滤。对集合类型中的每个元素按照某种规则进行过滤。
    reduce：计算，可以对数组中的元素进行计算。
```
//集合：map,filter,
var scores:Array<Int> = Array()
scores = [12,22,3,3444,221]
func isPassOrFail(score:Int)->String{
 return score>60 ? "Pass" : "Fail"
}


scores.map(isPassOrFail(score: ))
func fail(score:Int)->Bool{
 return score<60
}

scores.filter(fail(score: ))
